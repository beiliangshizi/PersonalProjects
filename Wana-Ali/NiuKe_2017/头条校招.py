...
# 题目描述
# 头条的2017校招开始了！为了这次校招，我们组织了一个规模宏大的出题团队，每个出题人都出了一些有趣的题目，而我们现在想把
# 这些题目组合成若干场考试出来，在选题之前，我们对题目进行了盲审，并定出了每道题的难度系统。一场考试包含3道开放性题目，
...
while i < n:
    ...
    if i + 1 >= n:
        r += j
        break
    if str1[i + 1] - str1[i] <= 10:
        j = 1
        if i + 2 >= n:
            r += j
            break
        if str1[i + 2] - str1[i + 1] <= 10:
            j = 0
    r += j
    i = i + 3 - j
